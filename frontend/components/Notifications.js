...
export function Notification({ id, type, message, duration = 5000, onClose, }) {
    useEffect(() => {
        if (duration) {
    ...
    return (<div className={`flex items-start gap-3 p-4 rounded-lg border ${bgColors[type]}`}>
      {icons[type]}
      <p className={`flex-1 text-sm ${textColors[type]}`}>{message}</p>
      <button onClick={() => onClose?.(id)} className="text-zinc-400 hover:text-zinc-300 flex-shrink-0">
        <X className="w-4 h-4"/>
      </button>
    </div>);
}
export function NotificationContainer({ notifications, onClose, }) {
    return (<div className="fixed top-4 right-4 space-y-2 z-50 max-w-md">
      {notifications.map(notif => (<Notification key={notif.id} {...notif} onClose={onClose}/>))}
    </div>);
}
...
export function useNotifications() {
    ...
    const error = (message) => add('error', message, 6000);
