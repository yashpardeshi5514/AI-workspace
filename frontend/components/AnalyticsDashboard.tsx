  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/enterprise/analytics?workspaceId=${workspaceId}&days=${days}`,
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          label="Avg Daily"
          value={Math.round(analytics.totalEvents / days)}
        />
        <StatCard
          icon={<Zap className="w-6 h-6" />}
          label="Events/User"
          value={Math.round(analytics.totalEvents / Math.max(analytics.activeUsers, 1))}
        />
      </div>
        ...
        {[7, 30, 90].map(d => (
          <button
            ...
            className={`px-4 py-2 rounded ${
              days === d ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'
            }`}
          >
            Last {d}d
          </button>
