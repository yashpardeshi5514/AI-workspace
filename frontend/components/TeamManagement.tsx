...
export function TeamManagement({ workspaceId }: TeamManagementProps) {
  ...
  const loadMembers = async () => {
    ...
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/enterprise/team?workspaceId=${workspaceId}`,
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/enterprise/team/invite`,
  ...
  const handleRemove = async (memberId: string) => {
    if (!token || !window.confirm('Remove team member?')) return;

    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/api/enterprise/team/${memberId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      await loadMembers();
    } catch (err) {
      ...
      <div className="bg-zinc-800 rounded-lg overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {members.map(member => (
                <tr key={member._id} className="border-b border-zinc-700 hover:bg-zinc-700/50">
