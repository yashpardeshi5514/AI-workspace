...
export function CodeRunner({ code, language = 'javascript' }) {
    ...
    const handleRun = async () => {
        ...
        setIsRunning(true);
        try {
            const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/execute/execute`, { code, language }, { headers: { Authorization: `Bearer ${token}` } });
            setResult(res.data);
        }
        catch (err) {
            setResult({
    ...
    return (<div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <button onClick={handleRun} disabled={isRunning} className="flex items-center gap-2 px-3 py-1 bg-green-600 hover:bg-green-700 disabled:opacity-50 rounded text-sm transition-colors">
          {isRunning ? (<>
      ...
      {result && (<div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
