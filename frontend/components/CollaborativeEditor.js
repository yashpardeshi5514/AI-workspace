'use client';
import { useEffect, useRef } from 'react';
import { io } from 'socket.io-client';
import { Editor } from '@monaco-editor/react';
export function CollaborativeEditor({ fileId, content, language = 'typescript', onChange, readOnly = false, }) {
    const socketRef = useRef(null);
    const editorRef = useRef(null);
    const [activeUsers, setActiveUsers] = React.useState([]);
    const currentUserRef = useRef('user-' + Date.now());
    useEffect(() => {
        // Initialize WebSocket
        socketRef.current = io(process.env.NEXT_PUBLIC_WS_URL || '', {
            path: '/socket.io',
        });
        const socket = socketRef.current;
        const userId = currentUserRef.current;
        // Join editor session
        socket.emit('join-editor', {
            fileId,
            userId,
            userName: 'User ' + userId.slice(-4),
        });
        // Listen for user changes
        socket.on('user-joined', (data) => {
            setActiveUsers(data.users);
        });
        socket.on('user-left', (data) => {
            setActiveUsers(data.users);
        });
        socket.on('cursor-updated', (cursor) => {
            // Draw remote cursor in editor (could use decorations)
        });
        socket.on('content-updated', (data) => {
            if (data.userId !== userId && editorRef.current) {
                // Update editor content from remote change
                editorRef.current.setValue(data.content);
            }
        });
        // Get active users
        socket.emit('get-users', { fileId });
        socket.on('active-users', (users) => {
            setActiveUsers(users);
        });
        return () => {
            socket.emit('leave-editor', { fileId, userId });
            socket.disconnect();
        };
    }, [fileId]);
    const handleEditorChange = (newContent) => {
        const content = newContent || '';
        // Send to others
        if (socketRef.current) {
            socketRef.current.emit('content-change', {
                fileId,
                userId: currentUserRef.current,
                newContent: content,
            });
        }
        onChange?.(content);
    };
    const handleEditorMount = (editor) => {
        editorRef.current = editor;
    };
    const handleCursorChange = () => {
        if (editorRef.current && socketRef.current) {
            const selection = editorRef.current.getSelection();
            socketRef.current.emit('cursor-move', {
                fileId,
                userId: currentUserRef.current,
                line: selection?.startLineNumber || 0,
                column: selection?.startColumn || 0,
            });
        }
    };
    return (<div className="flex flex-col h-full">
      {/* Active Users Bar */}
      <div className="flex items-center gap-2 px-4 py-2 bg-zinc-800 border-b border-zinc-700 text-xs">
        <span className="text-zinc-400">Editing:</span>
        {activeUsers.map(user => (<div key={user.userId} className="flex items-center gap-1 px-2 py-1 bg-zinc-700 rounded">
            <div className="w-2 h-2 rounded-full" style={{
                backgroundColor: hashColor(user.userId),
            }}/>
            <span>{user.userName}</span>
          </div>))}
      </div>

      {/* Editor */}
      <div className="flex-1">
        <Editor height="100%" language={language} value={content} onChange={handleEditorChange} onMount={handleEditorMount} onCursorPositionChange={handleCursorChange} options={{
            minimap: { enabled: false },
            readOnly,
            theme: 'vs-dark',
        }}/>
      </div>
    </div>);
}
// Helper to generate consistent colors for users
function hashColor(str) {
    const colors = [
        '#ff6b6b',
        '#4ecdc4',
        '#45b7d1',
        '#ffa502',
        '#ff006e',
        '#8338ec',
    ];
    let hash = 0;
    for (let i = 0; i < str.length; i++) {
        hash = str.charCodeAt(i) + ((hash << 5) - hash);
    }
    return colors[Math.abs(hash) % colors.length];
}
//# sourceMappingURL=CollaborativeEditor.js.map