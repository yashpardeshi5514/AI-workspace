...
export const handleAPIError = (error) => {
    if (error.response) {
        ...
        if (status === 422) {
            return new APIError(422, 'VALIDATION_ERROR', 'Please check your input and try again.', data.details);
        }
        if (status === 429) {
            return new APIError(429, 'RATE_LIMITED', 'Too many requests. Please try again later.');
        }
        if (status >= 500) {
            return new APIError(status, 'SERVER_ERROR', 'Server error. Please try again later.');
        }
        return new APIError(status, data.code || 'ERROR', data.message || 'An error occurred');
    }
    if (error.request) {
