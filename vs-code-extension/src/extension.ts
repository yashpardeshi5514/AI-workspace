...
export class AIWorkspaceExtension {
  ...
  activate() {
    ...
    this.context.subscriptions.push(
      vscode.commands.registerCommand('ai-workspace.openChat', () => {
        this.openChatPanel();
      })
    );
  ...
  private async executeSelectedCode() {
    ...
    try {
      const response = await axios.post(
        `${this.serverUrl}/api/execute/execute`,
        { code, language },
        {
  ...
  private sendSelectionToChat() {
    ...
    vscode.window.showInformationMessage(
      `Sent ${code.length} characters to chat`
    );
  }
