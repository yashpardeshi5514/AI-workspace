...
// List repositories
router.get('/repos/:username', authMiddleware, asyncHandler(async (req, res) => {
...
router.post('/clone', authMiddleware, asyncHandler(async (req, res) => {
...
// List files in repo
router.get('/repos/:owner/:repo/files', authMiddleware, asyncHandler(async (req, res) => {
    ...
    const service = new GitHubService(githubToken);
    const files = await service.listFiles(owner, repo, path, branch);
    res.json(files);
}));
// Get file content
router.get('/repos/:owner/:repo/file', authMiddleware, asyncHandler(async (req, res) => {
    const { owner, repo } = req.params;
    const { githubToken, path, branch = 'main' } = req.query;
    if (!githubToken || !path) {
    ...
    const service = new GitHubService(githubToken);
    const content = await service.getFileContent(owner, repo, path, branch);
    if (!content) {
...
// Create pull request
router.post('/pr/create', authMiddleware, asyncHandler(async (req, res) => {
...
// List issues
router.get('/repos/:owner/:repo/issues', authMiddleware, asyncHandler(async (req, res) => {
    const { owner, repo } = req.params;
...
// Create issue
router.post('/issues/create', authMiddleware, asyncHandler(async (req, res) => {
