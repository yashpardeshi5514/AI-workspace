...
async function connectDB() {
    try {
        await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/ai-workspace');
...
io.on('connection', (socket) => {
    ...
    socket.on('message:send', async (data) => {
        ...
        try {
            // Save user message
            const userMsg = await messageService.saveMessage(workspaceId, userId, content, 'user');
            ...
            const assistantMsg = await messageService.saveMessage(workspaceId, 'assistant', response, 'assistant');
            io.to(`workspace:${workspaceId}`).emit('message', {
                id: assistantMsg._id,
